...
@contextmanager
def github_cmd_line_auth(user, passwd):
    restore_netrc = False
    if os.path.exists('.netrc'):
        restore_netrc = True
        os.path.rename('.netrc', '.netrc.bak')
...
def bootstrap():
    ...
    try:
        ...
        with github_cmd_line_auth(github_user, github_password):
        ...
        for i in range(1800/sleep_time):
            ...
            if r.status_code == 200:
                ...
                break
            print 'got bad status code ({}). attempt={}, content={}'.format(r.status_code, i, r.content)
    finally:
        try:
            gh_repo.delete()
        except NameError:
            pass
        local('rm -rf {} {}'.format(proj_path, venv_path))
